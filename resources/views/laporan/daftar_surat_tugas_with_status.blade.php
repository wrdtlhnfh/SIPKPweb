		...
		<div class="card" style="overflow-x: auto">
			...
			<div class="card-body">
				<select name='status_proposal' id='status_proposal' class="form form-control" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
					<option value="{{url('surat_tugas')}}" selected>Semua</option>
					<option value="{{url('surat_tugas/0/status')}}" {{$status == 0 ? 'selected' : ''}}>Belum Dicetak</option>
					<option value="{{url('surat_tugas/1/status')}}" {{$status == 1 ? 'selected' : ''}}>Sudah Dicetak</option>
				</select>
				<br>
				<br>
