	...
	<div class="col-md-12">
		<div class="card" style="overflow-x: auto">
			<div class="card-header">
				<i class="fa fa-align-justify"></i> Data Mahasiswa Kerja Praktek
			</div>
			...
			<div class="card-body">
				<select name='status_kp' id='status_kp' class="form form-control" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
					<option value="{{url('lihat_status/kp')}}" selected="">Semua</option>
					
					@foreach($status_kps as $status_kp)
						<option value="{{url('lihat_status/kp/'.$status_kp->id)}}" {{$status_kp->id == $status ? 'selected' : ''}}>{{$status_kp->status_kp}}</option>
					@endforeach
				</select>
